...
void TProject::SetupAcceptanceCorrections()
{
	...
	else
	{
		...
		if(ProjectParameters->IsSettingDefined("path_to_acceptance_frame_graph"))
		{
			string path_to_acceptance_frame_graph =  ProjectParameters->GetSettingValue("path_to_acceptance_frame_graph") ;

			cout << "hereim path_to_acceptance_frame_graph " <<  path_to_acceptance_frame_graph << endl ;

			TFile *file_for_acceptance = TFile::Open(path_to_acceptance_frame_graph.c_str()) ;

			TGraph *acceptance_frame = ((TGraph *)file_for_acceptance->Get("Graph")) ;

			for(int i = 0 ; i < acceptance_frame->GetN() ; ++i)
			{
				double acx, acy ;
				acceptance_frame->GetPoint(i, acx, acy) ;

				xvalues.push_back(acx) ;
				yvalues.push_back(acy) ;
			}

			file_for_acceptance->Close() ;
		}
