...
// Class:      RecoAnalyzer
//
/**\class RecoAnalyzer RecoAnalyzer.cc
 /Projects/Glueball_analysis/plugins/RecoAnalyzer.cc

 Description: Processing code used in the glueball analysis
 Implementation: consult with the github repository
*/
//         Developed from PromptAnalyzer.cc by Michael Pitt
//         (michael.pitt@cern.ch) Created:  Thu, 20 Jan 2022 08:37:11 GMT
//
//
...
// Confirmed, uses the optics given in "Proton reconstruction using the TOTEM
// Roman pot detectors during the high-β ∗ data taking period"
...
// optics
// const double v_x_R_1_F = -2.24791387053766 ;  const double L_x_R_1_F =
// 0.125396407127792E3 ; const double v_y_R_1_F = +0.025781593410852 ; const
// double L_y_R_1_F = 238.517247191010E3 ; const double v_x_R_2_F =
// -1.92610996810677 ;  const double L_x_R_2_F = -3.00655323980445E3 ; const
// double v_y_R_2_F = -0.000000021508565 ; const double L_y_R_2_F =
// 271.511335947517E3 ;

// const double v_x_L_1_F =  -2.1964E+000  ;  const double L_x_L_1_F
// =   3.1581E+003 ; const double v_y_L_1_F =  +2.5777E-002  ;  const double
// L_y_L_1_F =   2.3834E+005 ; const double v_x_L_2_F =  -1.8739E+000  ;  const
// double L_x_L_2_F =  -4.9261E+002 ; const double v_y_L_2_F =  +2.7884E-010  ;
// const double L_y_L_2_F =   2.7157E+005 ;

const double v_x_R_1_F = v_x_R_1_F_matched;
const double L_x_R_1_F = L_x_R_1_F_matched;
const double v_y_R_1_F = v_y_R_1_F_matched;
const double L_y_R_1_F = L_y_R_1_F_matched;
const double v_x_R_2_F = v_x_R_2_F_matched;
const double L_x_R_2_F = L_x_R_2_F_matched;
const double v_y_R_2_F = v_y_R_2_F_matched;
const double L_y_R_2_F = L_y_R_2_F_matched;

const double v_x_L_1_F = v_x_L_1_F_matched;
const double L_x_L_1_F = L_x_L_1_F_matched;
const double v_y_L_1_F = v_y_L_1_F_matched;
const double L_y_L_1_F = L_y_L_1_F_matched;
const double v_x_L_2_F = v_x_L_2_F_matched;
const double L_x_L_2_F = L_x_L_2_F_matched;
const double v_y_L_2_F = v_y_L_2_F_matched;
const double L_y_L_2_F = L_y_L_2_F_matched;
...
RecoAnalyzer::RecoAnalyzer(const edm::ParameterSet &iConfig)
    : tracksToken_(consumes<TrackCollection>(
          iConfig.getUntrackedParameter<edm::InputTag>("tracks"))),
      DeDxDataToken_(consumes<DeDxDataValueMap>(
          iConfig.getUntrackedParameter<edm::InputTag>("DeDxData"))),
      ...
      genParticlesToken_(consumes<GenParticleCollection>(
...
void RecoAnalyzer::analyze(const edm::Event &iEvent,
  ...
  if (ev_.nvtx != 1)
    return; // use only events with 1 PV
  h_counter->Fill(2, 1);
  ...
  // cout << "Test event " << ev_.event << ", nvtx = " << ev_.nvtx << " , z = "
  // <<  ev_.zPV << endl;
  for (unsigned int i = 0; i < trackCollection.size(); i++) {
    ...
    ev_.trk_nMeasureLayer[i] +=
        1e2 * (p.pixelBarrelLayersWithMeasurement()); // PXB
    ev_.trk_nMeasureLayer[i] +=
        1e3 * (p.pixelEndcapLayersWithMeasurement()); // PXF
    ev_.trk_nMeasureLayer[i] +=
        1e4 * (p.stripTIBLayersWithMeasurement()); // TIB
    ev_.trk_nMeasureLayer[i] +=
        1e5 * (p.stripTIDLayersWithMeasurement()); // TID
    ev_.trk_nMeasureLayer[i] +=
        1e6 * (p.stripTOBLayersWithMeasurement()); // TOB
    ev_.trk_nMeasureLayer[i] +=
        1e7 * (p.stripTECLayersWithMeasurement()); // TEC

    const double ene_pi = sqrt((track->pt() * track->pt()) +
                               (track->pz() * track->pz()) + (m_pi * m_pi));
    const TLorentzVector trk_lorentz_pi(track->px(), track->py(), track->pz(),
                                        ene_pi);
  ...
  if (ev_.MAXTRACKS < ev_.ntrk) {
    cout << "ERROR: MAXTRACKS (" << ev_.MAXTRACKS
         << ") is smaller than the N of tracks (" << ev_.ntrk << ")." << endl;
  ...
  for (const auto &tr : *hRPTracks) {
    ...
    if (rpDecId == 4) {
      rp_valid_004 = true;
      ev_.xLN = tr.getX() + mean_x4;
      ev_.yLN = tr.getY() + mean_y4;
    }
    if (rpDecId == 5) {
      rp_valid_005 = true;
      ev_.xLN = tr.getX() + mean_x5;
      ev_.yLN = tr.getY() + mean_y5;
    }

    if (rpDecId == 24) {
      rp_valid_024 = true;
      ev_.xLF = tr.getX() + mean_x24;
      ev_.yLF = tr.getY() + mean_y24;
    }
    if (rpDecId == 25) {
      rp_valid_025 = true;
      ev_.xLF = tr.getX() + mean_x25;
      ev_.yLF = tr.getY() + mean_y25;
    }

    if (rpDecId == 104) {
      rp_valid_104 = true;
      ev_.xRN = tr.getX() + mean_x104;
      ev_.yRN = tr.getY() + mean_y104;
    }
    if (rpDecId == 105) {
      rp_valid_105 = true;
      ev_.xRN = tr.getX() + mean_x105;
      ev_.yRN = tr.getY() + mean_y105;
    }

    if (rpDecId == 124) {
      rp_valid_124 = true;
      ev_.xRF = tr.getX() + mean_x124;
      ev_.yRF = tr.getY() + mean_y124;
    }
    if (rpDecId == 125) {
      rp_valid_125 = true;
      ev_.xRF = tr.getX() + mean_x125;
      ev_.yRF = tr.getY() + mean_y125;
    }
  }
  ...
  // if((TMath::Abs(ev_.ThyL+ev_.ThyR)< 15e-6) &&
  // (TMath::Abs(ev_.ThxL+ev_.ThxR)<45e-6)) isElastic = true ;

  if (applyFilt_ && isElastic)
    return;
  h_counter->Fill(3, 1);
  ...
  if (!ev_.isData) {
    ...
    for (size_t i = 0; i < genParticles->size(); ++i) {
      ...
      if (genIt.status() != 1)
        continue; // save only stable particles
      if (genIt.pdgId() == 22)
  ...
  if (ev_.MAXGENTRACKS < ev_.ngentrk) {
    cout << "ERROR: MAXGENTRACKS (" << ev_.MAXGENTRACKS
         << ") is smaller than the N of tracks (" << ev_.ngentrk << ")."
         << endl;
    cout << "\t\t... can cause memory leaks or crush!!!" << endl;
  }
...
void RecoAnalyzer::genAnalysis(const edm::Event &iEvent,
                               const edm::EventSetup &iSetup) {
...
// module  ------------
void RecoAnalyzer::fillDescriptions(
