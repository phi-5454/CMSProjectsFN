...
#inter_dir="/eos/user/y/yelberke/intermediate/testCondorIndiv/ready"
inter_dir="/eos/user/y/yelberke/intermediate/condorIndivLoosecut/combined"
...
ntuplize_parallel() {
	...
	#echo "$totem_files"
	find "$inter_dir" -type f -name "TOTEM*.root" -exec basename {} \; | parallel -j $njobs  --ungroup analysisWrapper --skimtree --in "$inter_dir/"{} --out "$outdir"/"$analysis_name"/{} --method GlueballAnalysis
}
	...
	case $1 in
		...
		--inter-dir) 
			if [[ -n "$2" && ! "$2" =~ ^-- ]]; then
				...
				echo "Error: --inter-dir requires a non-empty option argument."
				usage
			fi
			;;
		--n-analyze)
